// PL_Project2.cpp : 此檔案包含 'main' 函式。程式會於該處開始執行及結束執行。
// 
...
/*
  ...
  大概概念係Project 1 有兩大部分: Parser 和 Scanner
...
class Symbol_Struct
{
  ...
  string msymbol_id ; // Symbol的名
  string msymbol_type ; // Symbol 的形態 有 NORMAL / SYSTEM_PRIMITIVES 

  // 只有NORMAL有symbol_binding
  Node* msymbol_binding ; // Symbol binding的Node

  // 只有SYSTEM_PRIMITIVES有
  string msystem_primitives_id ; 
} ; // Symbol_Struct
...
class Scanner
{
  ...
  char Get_next_char() // 讀入下個char(讀走space) 
  {
    ...
    else 
    {
      ...
      // mCurrent_Line = temp_line_counter ;
      // mCurrent_Column += 1 ;
      return next_char;
    } // else 
    /*
    ...
    {
      next_char = cin.get(); // 讀第1個有效字元
      if ( have_new_line == true && is_only_waste_in_line == false )
      {
        // 係新既一行而且係有有效字元
        temp_line_counter++;
        temp_column_counter++;
        mCurrent_Column = temp_column_counter;
        mCurrent_Line = temp_line_counter + mCurrent_Line;
        have_new_line = false;
        is_only_waste_in_line = false;
      } // if
      else if ( have_new_line == false && is_only_waste_in_line == false )
      {
        // 單純遇到非空白 既沒有換行也沒有空白
        if ( mCurrent_Line == 0 )
        {
        ...
        temp_column_counter++;
        mCurrent_Column = mCurrent_Column + temp_column_counter;
        have_new_line = false;
        is_only_waste_in_line = false;
      } // else if 
      else if ( have_new_line == true && is_only_waste_in_line == true )
      {
        // 在遇到有效字元前,先遇到1至多行沒有意義的行
        temp_column_counter++;
        mCurrent_Column = mCurrent_Column + temp_column_counter;
        mCurrent_Line = temp_line_counter + mCurrent_Line;
        have_new_line = false;
  ...
  Form_Of_Token Set_Form_Token( string input_token, string input_token_type )
  {
    ...
    for ( int i = 0; i < input_token.size() ; i++ )
    {
      ...
      // cout << "temp_token.mToken_Char : " << temp_token.mToken_Char  << endl ;
      // cout << "mCurrent_Column : " << mCurrent_Column << endl ;
      temp_token.mToken_COLUMN = ( ( mCurrent_Column - input_token.size() ) + ( i + 1 ) );
      // cout << "mCurrent_Column2 : " << mCurrent_Column << endl ;
  ...
  Form_Of_Token Def_Token( char first_char ) // 根據first_char決定它是甚麼Token
  {
    ...
    if ( first_char == '(' ) //  現時可能是LEFT_PAREN, LEFT_PAREN_RIGHT_PAREN
    {
      ...
      {
        // cout << "mCurrent_Column3: " << mCurrent_Column << endl ; 
        mCurrent_Column += 1 ;
        mCurrent_Column = mCurrent_Column - mScanner_Token_Column ; 
        // cout << "mCurrent_Column4 : " << mCurrent_Column << endl ; 
        cin.putback( buffer );
        buildup_Token = first_char;
        // 建構Token
        // cout << "mCurrent_Column yo: " << mCurrent_Column << endl ;
  ...
  Form_Of_Token Peek_Next_Token() // 偷看下個Token *主要Function*
  {
    ...
    // cout << "Current_Column : " << mCurrent_Column << endl;
    readin_Token = Def_Token( first_char );
    // int i = 0 ;
    // cout << "Here is \n";
    // cout << readin_Token.Token_ID() << endl;
    /*
    while ( i < readin_Token.mToken.size() )
    {
      cout << readin_Token.mToken[i].mToken_Char << endl;
      cout << "Current_Line : " << readin_Token.mToken[i].mToken_LINE << endl;
      cout << "Current_Column : " << readin_Token.mToken[i].mToken_COLUMN << endl;
      i++;
    } // while
    */
    
    // 先Peek完後,先放在mBuffer_Token_Set中
    // cout << "The type is " << readin_Token.mToken_TYPE << endl;
...
class Parser
{
  ...
  vector<Symbol_Struct> mdefined_SYMBOL_Binding ; // 儲存已被定義的SYMBOL
  ...
  void CheckSchemeGrammer( Form_Of_Token temp_token ) // 檢查文法
  {
    ...
    string num_Line;
    string num_Column;
    // cout << "Start Check Grammer\n" ;
    if ( Is_SExp( temp_token ) == true )
    {
      /*
      for ( int i = 0; i < scanner.mBuffer_Token.size(); i++ )
      {
        cout << scanner.mBuffer_Token[i].Token_ID() << " " ;
      } // for 
      */
      // cout << "\nThe Grammer is OK\n" ;
            ...
            if ( temp2->mright != NULL )
            {
              ...
              if ( temp2->mright == temp )
              {
                ...
                if ( temp->mright == node )
                {
                  ...
                  {
                    if ( SYSTEM_PRIMITIVES_Checker( node->minformation ) )
                    {
                      cout << "#<procedure " << node->minformation.Token_ID() << ">\n" ;
                    } // if 
                    else 
                    {
  ...
  Node* Car( Node* input_node )
  {
    ...
    if ( input_node->mstruct_type == "STRUCT" )
    {
      if ( input_node->mleft != NULL )
      {
        if ( input_node->mleft->mstruct_type == "QUOTE" ) 
        {
          if ( input_node->mright != NULL )
          {
            if ( input_node->mright->mstruct_type == "STRUCT" )
            {
              temp_node = input_node->mright ;
              return temp_node->mleft ;
            } // if 
          } // if 
        } // if 
        else 
  ...
  bool SYSTEM_PRIMITIVES_Checker( Form_Of_Token input )
  {
    if ( input.Token_ID() == "cons" 
         || input.Token_ID() == "list" 
         || input.Token_ID() == "quote"
         || input.Token_ID() == "'"
         || input.Token_ID() == "define"
         || input.Token_ID() == "car"
         || input.Token_ID() == "cdr"
         || input.Token_ID() == "atom?"
         || input.Token_ID() == "pair?"
         ...
         || input.Token_ID() == "real?" 
         || input.Token_ID() == "number?"
         || input.Token_ID() == "string?" 
         || input.Token_ID() == "boolean?"
         || input.Token_ID() == "symbol?"
         || input.Token_ID() == "+"
         || input.Token_ID() == "-"
         || input.Token_ID() == "*"
         || input.Token_ID() == "/" 
         ...
         || input.Token_ID() == "<" 
         || input.Token_ID() == "<="
         || input.Token_ID() == "string-append"
         || input.Token_ID() == "string>?"
         || input.Token_ID() == "string<?"
         || input.Token_ID() == "string=?"
         || input.Token_ID() == "eqv?"
         || input.Token_ID() == "equal?"
         || input.Token_ID() == "begin"
         || input.Token_ID() == "if"
         || input.Token_ID() == "cond"
         || input.Token_ID() == "clean-environment" )
    {
      return true ;
    } // if 
    else 
  ...
  } // SYSTEM_PRIMITIVES_Checker()

  void Binding_SYSTEM_PRIMITIVES()
  {
    // 用作把全部SYSTEM_PRIMITIVES和其功能作Binding
    Symbol_Struct temp_symbol_struct ;

    // cons
    temp_symbol_struct.msymbol_id = "cons" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "cons" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    
    // list
    temp_symbol_struct.msymbol_id = "list" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "list" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // quote
    temp_symbol_struct.msymbol_id = "quote" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "quote" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // '
    temp_symbol_struct.msymbol_id = "'" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "quote" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // define
    temp_symbol_struct.msymbol_id = "define" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "define" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // car
    temp_symbol_struct.msymbol_id = "car" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "car" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // cdr
    temp_symbol_struct.msymbol_id = "cdr" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "cdr" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // atom?
    temp_symbol_struct.msymbol_id = "atom?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "atom?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // pair?
    temp_symbol_struct.msymbol_id = "pair?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "pair?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    ...
    // null?
    temp_symbol_struct.msymbol_id = "null?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "null?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // integer?
    temp_symbol_struct.msymbol_id = "integer?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "integer?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // real?
    temp_symbol_struct.msymbol_id = "real?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "real?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // number?
    temp_symbol_struct.msymbol_id = "number?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "number?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // string?
    temp_symbol_struct.msymbol_id = "string?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "string?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // boolean?
    temp_symbol_struct.msymbol_id = "boolean?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "boolean?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // symbol?
    temp_symbol_struct.msymbol_id = "symbol?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "symbol?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // +
    temp_symbol_struct.msymbol_id = "+" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "+" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    ...
    // not
    temp_symbol_struct.msymbol_id = "not" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "not" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // and
    temp_symbol_struct.msymbol_id = "and" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "and" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // or
    temp_symbol_struct.msymbol_id = "or" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "or" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    ...
    // string-append
    temp_symbol_struct.msymbol_id = "string-append" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "string-append" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    ...
    // eqv?
    temp_symbol_struct.msymbol_id = "eqv?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "eqv?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // equal?
    temp_symbol_struct.msymbol_id = "equal?" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "equal?" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // begin
    temp_symbol_struct.msymbol_id = "begin" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "begin" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // if
    temp_symbol_struct.msymbol_id = "if" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "if" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // cond
    temp_symbol_struct.msymbol_id = "cond" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "cond" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;

    // clean-environment
    temp_symbol_struct.msymbol_id = "clean-environment" ;
    temp_symbol_struct.msymbol_type = "SYSTEM_PRIMITIVES" ;
    temp_symbol_struct.msystem_primitives_id = "clean-environment" ;
    mdefined_SYMBOL_Binding.push_back( temp_symbol_struct ) ;
    
  } // Binding_SYSTEM_PRIMITIVES 

  Node* EvalSExp( Node* inSExp )
  {
    // 只有合符文法的SExp 會進這Function做計算

    Form_Of_Token temp_token ; // 作暫存之用
    bool is_symbol_bound = false ;
    bool is_mdefined_SYMBOL_Binding_empty = false ;
    int i = 0 ;
    string error_message ; // 用作儲Error Message
    string temp_token_id ; // 用作暫存token_id

    if ( inSExp->mstruct_type == "INFORMATION" )
    {
      // 只是一個INFORMATION
      temp_token = inSExp->minformation ;
      if ( Is_ATOM( temp_token ) )
      {
        // 是ATOM
        if ( temp_token.mToken_TYPE != "SYMBOL" )
        {
          // 直接return 該ATOM
          return inSExp ; // 把它自已回傳
        } // if 
        else if ( temp_token.mToken_TYPE == "SYMBOL" )
        {
          // 下面檢查是不是可以被redefine的SYMBOL
          
          if ( SYSTEM_PRIMITIVES_Checker( temp_token ) == false )
          {
            // 可以被redefine的SYMBOL
            if ( mdefined_SYMBOL_Binding.empty() )
            {
              // 現時沒有SYMBOL被定義
              // error type 5: ERROR (unbound symbol) : temp_token.Token_ID() 
              temp_token_id = temp_token.Token_ID() ;
              error_message = "ERROR (unbound symbol) : " + temp_token_id ;
              throw error_message ;
            } // if 
            else 
            {
              // 檢查該SYMBOL是否被定義
              while( is_symbol_bound == false && ( i <= mdefined_SYMBOL_Binding.size() ) )
              {
                // 檢查是否被定義過
                if ( i > mdefined_SYMBOL_Binding.size() )
                {
                  // 看完全部 該SYMBOL沒有被定義過
                  is_symbol_bound = false ;
                } // if 
                else if ( mdefined_SYMBOL_Binding[i].msymbol_id == temp_token.Token_ID() )
                {
                  // 該SYMBOL有被定義過
                  is_symbol_bound = true ;
                } // else if 
                else 
                {
                  // 該SYMBOL沒有被定義過
              ...
              if ( is_symbol_bound == true )
              {
                // return 該SYMBOL的Binding
                return mdefined_SYMBOL_Binding[i].msymbol_binding ;
              } // if 
              else if ( is_symbol_bound == false && i > mdefined_SYMBOL_Binding.size() )
              {
                // error type 5: ERROR (unbound symbol) : temp_token.Token_ID() 
                temp_token_id = temp_token.Token_ID() ;
                error_message = "ERROR (unbound symbol) : " + temp_token_id ;
      ...
      {
        cout << "You are not ATOM\n" ;
      } // else 
    } // if 
    else if ( inSExp->mstruct_type == "NIL" ) 
    {
      return inSExp ; // 把它自已回傳
    } // else if 
    else if ( inSExp->mstruct_type == "STRUCT" )
    {
      // 主要指令判斷
...
int main()
{
  ...
  project.Binding_SYSTEM_PRIMITIVES() ; // 建構SYSTEM_PRIMITIVES 內部指令
  char uTestNum;
  uTestNum = cin.get();
  uTestNum = cin.get();
  ...
  while ( end_Of_Input != true )
  {
    try
    {
      ...
      result_node = project.CreateStructNode() ;
      root->mparent = NULL ;
      result_node->mparent = NULL ;
      ...
      */
      ...
      result_node->mleft = project.EvalSExp( root->mleft ) ;
      // cout << result_node->minformation.Token_ID() << endl ;
      project.PrettyPrint( result_node->mleft, space_counter ) ;
